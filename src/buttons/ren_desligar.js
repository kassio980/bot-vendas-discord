...
await i.deferUpdate();try{const r=await ren.desligar();const b=JSON.parse(r.raw||'{}');
await i.followUp({embeds:[info('✅ COMANDO ENVIADO','Serviço desligando em instantes\nStatus: `'+(b.service?.status||r.status)+'`')],ephemeral:true})
