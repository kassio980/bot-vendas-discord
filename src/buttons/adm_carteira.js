...
function painelSecundario() {
  return new ActionRowBuilder().addComponents(
    ...
    new ButtonButton = new ButtonBuilder().setCustomId(baseId+'config').setLabel('⚙️ Config').setStyle(ButtonStyle.Primary)
  );
}

module.exports = { id: 'MINION_STORE_carteira', async execute(c,i) {
  const arr = Array.isArray(b.ler('carteira')) ? b.ler('carteira') : [];
  const total = arr.reduce(function(s,x){ return s + (x.saldo||0); }, 0);
  await i.update({ embeds:[emb.info('💵 CARTEIRA DE CLIENTES', 'Saldo total distribuido: **R$ '+total.toFixed(2).replace('.',',')+'**\nVocê pode adicionar ou remover saldo de qualquer cliente')], components:[painelPrincipal(), painelSecundario()] });
}};