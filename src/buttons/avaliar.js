const {ler,salvar,addLog}=require('../../banco');const {obrigadoAvaliacao}=require('../../embeds');
...
avs.unshift({pedido:pid,cliente:i.user.id,nota,produto:p.produtoId,data:new Date().toISOString()});
salvar('avaliacoes',avs);
addLog('alteracoes',{acao:'AVALIACAO',pedido:pid,nota});
const e=obrigadoAvaliacao(p,nota);
