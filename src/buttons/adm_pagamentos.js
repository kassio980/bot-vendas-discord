...
module.exports = { id: 'MINION_STORE_pagamentos', async execute(c,i) {
  const pendentes = (Array.isArray(b.ler('pedidos')) ? b.ler('pedidos') : []).filter(function(x){ return x.status==='AGUARDANDO'; }).length;
  await i.update({ embeds:[emb.info('💳 GERENCIAR PAGAMENTOS', 'Pagamentos aguardando aprovação: **'+pendentes+'**\nClique em um pedido para aprovar ou recusar')], components:[painelPrincipal(), painelSecundario()] });
}};