...
const {iniciarMonitorVoz,conectarVoz,statusVoz}=require('../../voz');
const ren=require('../../render_api');
module.exports={
data:new SlashCommandBuilder().setName('sistema').setDescription('⚙️ Controle total do sistema')
.addSubcommand(s=>s.setName('render').setDescription('🔌 Gerenciar hospedagem'))
.addSubcommand(s=>s.setName('voz').setDescription('🔊 Conectar/monitorar voz'))
...
if(!PERM)return i.reply({embeds:[erro('ACESSO NEGADO','Apenas o DONO principal pode usar o /sistema.')],ephemeral:true});
const sb=i.options.getSubcommand();
if(sb==='voz'){const v=statusVoz();iniciarMonitorVoz(c);return i.reply({embeds:[sucesso('MONITOR DE VOZ ATIVADO','Status atual: '+(v.conectado?'✅ Conectado em <#'+v.canal+'>':'🔁 Tentando conectar...\n\nConfirme se a variavel **CANAL_VOZ_ID** esta correta no Render.')],ephemeral:true})}
if(sb==='render'){let st='offline';try{const s=await ren.status();st=s.json().service?.status||'verificando'}catch{}
...
return i.reply({embeds:[sucesso('ADM ADICIONADO','<@'+u.id+'> agora faz parte da equipe administrativa!')],ephemeral:true})}
