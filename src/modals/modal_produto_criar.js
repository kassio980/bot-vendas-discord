...
p.push({id:gerarId(),nome:g('nome'),descricao:g('descricao'),preco,estoque,tipoEntrega:T.includes(entrega)?entrega:'mensagem',dataCriacao:new Date().toISOString()});
salvar('produtos.json',p);addLog('alteracoes',{acao:'PRODUTO_CRIADO',nome:g('nome')});
await i.reply({embeds:[sucesso('CRIADO!',`**${g('nome')}**\nR$ ${preco.toFixed(2)}`)],ephemeral:true});
